...
function createStore(storeParams) {
  ...
  const originalState = __spreadValues({}, storeParams.state || {});
  const actions = __spreadValues({}, storeParams.actions || {});
  const getters = __spreadValues({}, storeParams.getters || {});
  ...
  const methods = extend(ConstructorMethods({
    ...
    open(el, animate, targetEl) {
      ...
      if (!instance) {
        ...
        instance = new Constructor(app, __spreadValues({
          el: $el
        }, params));
      }
...
var resize_default = {
  ...
  create() {
    ...
    app.getSize = () => {
      ...
      const [width, height, left, top] = [app.el.offsetWidth, app.el.offsetHeight, offset2.left, offset2.top];
      app.width = width;
      app.height = height;
      app.left = left;
      app.top = top;
...
function modalLoad(modalType, route, loadOptions, direction) {
  ...
  const routeCallback = (modal, name) => {
    ...
    let callback;
    if (name === "open") {
      callback = on2.modalOpen || once.modalOpen || on2.panelOpen || once.panelOpen;
    }
    if (name === "close") {
      callback = on2.modalClose || once.modalClose || on2.panelClose || once.panelClose;
    }
    if (name === "closed") {
      callback = on2.modalClosed || once.modalClosed || on2.panelClosed || once.panelClosed;
    }
    if (callback)
      callback(modal);
  };
...
//# sourceMappingURL=chunk-A4A57DTQ.js.map
